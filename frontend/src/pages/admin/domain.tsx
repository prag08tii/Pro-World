...
import { Plus, Edit2, Trash2, Globe, X, ChevronDown } from "lucide-react";
...
export default function Domains() {
  ...
  const fetchDomains = async () => {
    try {
      ...
      const res = await API.get(`/domains/?${query}`);
      setDomains(res.data);
    } catch {
  ...
  const handleUpdate = async () => {
    try {
      await API.put(`/domains/${editing.id}/`, editing);
  ...
  const handleDelete = async () => {
    try {
      await API.delete(`/domains/${confirmDelete.id}/`);
    ...
    <AdminLayout>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-0.5">{domains.length} domains listed</p>
            </div>
            <button
            ...
            {domains.length === 0 ? (
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-50">
                  {domains.map((d, i) => (
                    <tr key={d.id} className={`hover:bg-blue-50/30 transition-colors ${i % 2 !== 0 ? "bg-gray-50/30" : ""}`}>
                      <td className="px-6 py-4 text-sm text-gray-400 font-mono w-16">#{d.id}</td>
                      ...
                      <td className="px-6 py-4 text-sm text-gray-500 max-w-xs">
                        <p className="truncate">{d.description || <span className="text-gray-300 italic">No description</span>}</p>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <button onClick={() => setEditing(d)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-600 text-xs font-semibold transition-colors">
                            <Edit2 size={12} /> Edit
                          </button>
                          <button onClick={() => setConfirmDelete(d)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-50 hover:bg-red-100 text-red-600 text-xs font-semibold transition-colors">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
              <h3 className="font-bold text-gray-900">Add Domain</h3>
            ...
            <div className="p-6 space-y-3">
              <input placeholder="Domain Name" onChange={e => setForm({ ...form, name: e.target.value })} className={inputClass} />
              <textarea placeholder="Description (optional)" onChange={e => setForm({ ...form, description: e.target.value })} rows={3} className={`${inputClass} resize-none`} />
            </div>
      ...
      {editing && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
              <h3 className="font-bold text-gray-900">Edit Domain</h3>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
            ...
            <h3 className="font-bold text-gray-900 mb-1">Delete Domain?</h3>
